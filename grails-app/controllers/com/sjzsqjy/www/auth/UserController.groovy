...
class UserController extends AbstractS2UiController {
	...
	def save = {
        ...
        def userClinic = new UserClinic(hosUser: user,clinic:clinic)
        if (!userClinic.save(flush:true)) {
            userClinic.errors.each {
                println it
            }
        }
	...
	def userSearch = {
		...
		def hql = new StringBuilder('FROM ').append(lookupUserClassName()).append(' u,UserClinic uc WHERE 1=1 and u.id=uc.hosUser.id ')
		def queryParams = [:]
		...
		def results = lookupUserClass().executeQuery(
				"SELECT u.id,u.username,u.enabled,uc.clinic.name $hql $orderBy",
				queryParams, [max: max, offset: offset])
//        results.add( lookupUserClass().executeQuery(
//                "SELECT u.id,u.username,u.enabled,null from "+lookupUserClassName()+" as u  $orderBy",
//                queryParams, [max: max, offset: offset]))
	...
	protected Map buildUserModel(user) {
        ...
        def clinicName = Clinic.executeQuery("select uc.clinic.name from UserClinic uc where uc.hosUser.id=? ",[user.id])
		return [user: user, roleMap: granted + notGranted,clinicName:clinicName]
	}
