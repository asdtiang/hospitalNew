...
class RegisterController extends AbstractS2UiController {
	...
	def register = { RegisterCommand command ->
		...
		String password = springSecurityService.encodePassword(command.password, saltSource instanceof NullSaltSource ? null : command.username)
		def user = new HosUser(email: command.email, username: command.username,
				accountLocked: false, enabled: true,password: password)
		...
		println "user.password:"+user.password
		def userAuthority =HosUserHosRole.create(user,HosRole.findByAuthority("ROLE_USER"))
