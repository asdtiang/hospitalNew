...
<table width="650" style="border-collapse: collapse; border: solid #000000; border-width: 0px 0 1px 0px;">
	...
	<g:each in="${resultList}" status="i" var="resultInstance">
		<tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
			...
			${fieldValue(bean: resultInstance, field: "result")}
			</td>			           
			<td style="font-size:12px;">
			<g:if test="${sex==1}">	
			${resultInstance.normalList1[i][0]}---${resultInstance.normalList1[i][1]}
			</g:if>
			<g:else>${resultInstance.normalList[i][0]}---${resultInstance.normalList[i][1]}</g:else>
